...
Details for our approach of calculating the site-specific z-scores and the yearly averages are as follows and are presented graphically in `r fig_num(name = "z_schematic", caption = "Example calculation of the site-specific z-scores and yearly mean z-scores.", display = "cite")`.  Additionally, an example R script, `schematic.R` and example dataset, `schematic.csv` to recreate and demonstrate these calculations is available from <https://github.com/jhollist/ri_wq_trends>.  The general steps, listed below, are repeated for each of the water quality parameters.

WRONG ORDER JEFF!
1. For each site, calculate the reference mean and reference standard deviation for all values.
2. Calculate the z-score for each measurement by subtracting the reference mean and dividing by the reference standard deviation.
3. Summarize the z-scores by calculating an average z-score for each site in each year.  This step prevent bias from pseudoreplication of multiple measurements of the same site in a given year ([@hurlbert1984pseudoreplication]).  The same site averages across years are assumed to be independent.
4. Summarize the yearly z-scores by calculating the average, standard deviation, and sample size of the per site z-scores.  

The resultant values are analyzed for a trend over time.
...
![](../figures/paired_chla_trends.jpg)
...
![](../figures/paired_tn_trends.jpg)

*`r fig_num(name = "tp_trend")`*

![](../figures/paired_tp_trends.jpg)

*`r fig_num(name = "np_trend")`*

![](../figures/paired_np_trends.jpg)
